...
const SEED_TASKS = [
  ...
  },
  {
    ...
    setupCommands: [
      ...
      'pgrep -x sshd >/dev/null 2>&1 || { systemctl stop ssh.socket >/dev/null 2>&1 || true; mkdir -p /run/sshd; /usr/sbin/sshd; }',
    ],
    validationRules: [
      { type: 'command_history_contains', label: 'Show the eth0 interface with ip a / ip addr show', params: { command: 'ip a', aliases: ['ip addr', 'ip addr show', 'ip address'] } },
      { type: 'command_history_contains', label: 'Show IPv4 addresses with ip a (inet lines)', params: { command: 'ip a', aliases: ['ip addr', 'ip addr show', 'ip address'] } },
      { type: 'command_history_contains', label: 'The classic ifconfig must also list eth0', params: { command: 'ifconfig' } },
      { type: 'command_history_contains', label: 'ping -c 3 127.0.0.1 must receive replies', params: { command: 'ping -c 3 127.0.0.1' } },
      { type: 'command_history_contains', label: 'traceroute -n 127.0.0.1 must reach 127.0.0.1', params: { command: 'traceroute -n 127.0.0.1' } },
      { type: 'command_history_contains', label: 'app.linuxlab.local must resolve to 127.0.0.1', params: { command: 'getent hosts app.linuxlab.local', aliases: ['nslookup app.linuxlab.local', 'dig app.linuxlab.local', 'host app.linuxlab.local'] } },
      { type: 'command_history_contains', label: 'app.linuxlab.local must be reachable over ICMP', params: { command: 'ping -c 2 app.linuxlab.local' } },
      { type: 'command_history_contains', label: 'ss -tulpn must show sshd listening on port 22', params: { command: 'ss -tulpn', aliases: ['netstat -tulpn'] } },
