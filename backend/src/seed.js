...
const SEED_TASKS = [
  ...
  },
  {
    ...
    validationRules: [
      ...
      { type: 'file_permissions', label: '/usr/local/bin/backup.sh is executable (755)', params: { path: '/usr/local/bin/backup.sh', expected: '755' } },
      { type: 'file_contains', label: '/usr/local/bin/backup.sh uses tar for the backup', params: { path: '/usr/local/bin/backup.sh', needle: 'tar' } },
  ...
  },
  {
    ...
    validationRules: [
      { type: 'file_contains', label: '/etc/ssh/sshd_config sets PermitRootLogin no', params: { path: '/etc/ssh/sshd_config', needle: 'PermitRootLogin no' } },
  ...
  },
  {
    ...
    validationRules: [
      ...
      { type: 'file_permissions', label: '/opt/banking/config.conf is restricted to 640', params: { path: '/opt/banking/config.conf', expected: '640' } },
      { type: 'file_permissions', label: '/opt/banking/app.sh is executable (755)', params: { path: '/opt/banking/app.sh', expected: '755' } },
      { type: 'file_permissions', label: '/opt/banking/deploy.sh is executable (755)', params: { path: '/opt/banking/deploy.sh', expected: '755' } },
      { type: 'file_permissions', label: '/opt/banking/backup.sh group/other write removed (755)', params: { path: '/opt/banking/backup.sh', expected: '755' } },
      { type: 'file_permissions', label: '/opt/banking/users.csv stays at 644', params: { path: '/opt/banking/users.csv', expected: '644' } },
      { type: 'file_permissions', label: '/opt/banking/reports is group-writable (775)', params: { path: '/opt/banking/reports', expected: '775' } },
      { type: 'file_permissions', label: '/opt/banking/logs is group-writable (775)', params: { path: '/opt/banking/logs', expected: '775' } },
      { type: 'file_permissions', label: '/opt/banking/scripts has the SGID bit (2775)', params: { path: '/opt/banking/scripts', expected: '2775' } },
      { type: 'command_contains', label: '/opt/banking/scripts shows SGID as drwxrwsr-x', params: { command: 'ls -ld /opt/banking/scripts', needle: 'drwxrwsr-x' } },
      { type: 'file_permissions', label: '/opt/banking/shared is 775', params: { path: '/opt/banking/shared', expected: '775' } },
      { type: 'file_permissions', label: '/opt/banking/uploads has the sticky bit (1777)', params: { path: '/opt/banking/uploads', expected: '1777' } },
      { type: 'command_contains', label: '/opt/banking/uploads shows sticky bit as drwxrwxrwt', params: { command: 'ls -ld /opt/banking/uploads', needle: 'drwxrwxrwt' } },
    ],
    sections: [
      {
  ...
  },
  {
    ...
    validationRules: [
      { type: 'file_exists', label: '/srv/webapp/config.yml exists', params: { path: '/srv/webapp/config.yml' } },
      { type: 'file_type', label: '/srv/webapp/config.yml is a regular file (hard link, not a symlink)', params: { path: '/srv/webapp/config.yml', expected: 'regular file' } },
      { type: 'hardlink_exists', label: '/srv/webapp/config.yml and /srv/webapp/settings.yml are the same file (same inode)', params: { a: '/srv/webapp/config.yml', b: '/srv/webapp/settings.yml' } },
      { type: 'file_linkcount', label: '/srv/webapp/config.yml has a link count of 2', params: { path: '/srv/webapp/config.yml', expected: '2' } },
      { type: 'file_contains', label: '/srv/webapp/config.yml exposes the settings content', params: { path: '/srv/webapp/config.yml', needle: 'app_port: 8080' } },
      { type: 'symlink_exists', label: '/srv/webapp/www/assets is a symbolic link', params: { path: '/srv/webapp/www/assets' } },
      { type: 'dir_exists', label: '/srv/webapp/www/assets resolves to the shared-assets directory', params: { path: '/srv/webapp/www/assets' } },
      { type: 'file_exists', label: '/srv/webapp/www/assets/logo.png is reachable through the symlink', params: { path: '/srv/webapp/www/assets/logo.png' } },
      { type: 'file_contains', label: '/srv/webapp/www/assets/logo.png content is reachable through the symlink', params: { path: '/srv/webapp/www/assets/logo.png', needle: 'GLBX-LOGO-DATA' } },
    ],
    sections: [
      {
